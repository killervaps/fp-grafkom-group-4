...
const debugName = document.getElementById('object-name');
const debugDist = document.getElementById('object-dist');
...
const collisionRaycaster = new THREE.Raycaster(new THREE.Vector3(), new THREE.Vector3(), 0, playerRadius);
...
function init() {
    ...
    camera.position.set(-23.427, 19.0, 49.810);
    ...
    directionalLight.position.set(5, 74.260, 7.480);
    ...
    loader.load(
        ...
        function (gltf) {
            ...
            console.log('Distance from camera to model center:', camera.position.distanceTo(center).toFixed(2));
            ...
            console.log(`Collision system ready with ${collidableObjects.length} collidable objects`);
        },
        function (xhr) {
...
function updateRaycaster() {
    ...
    if (!loadedModel) {
        debugName.innerText = "Loading...";
        debugName.style.color = "#aaa";
        debugDist.innerText = "Distance: -";
        return;
    }
    ...
    if (intersects.length > 0) {
        ...
        const materialName = objectHit.material?.name || 'No Material';

        // Tampilkan nama di HTML Debug
        const displayName = objectHit.name || `${objectType}`;
        debugName.innerText = `${displayName}`;
        debugName.style.color = "#0f0"; // Hijau jika kena
        debugDist.innerText = `Dist: ${distance.toFixed(2)}m | Mat: ${materialName}`;
        ...
        debugName.innerText = "No Object";
        debugName.style.color = "#888";
        debugDist.innerText = "Distance: -";
    }
}
...
function checkCollision(moveVector) {
    ...
    for (let i = 0; i < collisionDirections.length; i++) {
        ...
        const intersections = collisionRaycaster.intersectObjects(collidableObjects, false);
        ...
        if (intersections.length > 0 && intersections[0].distance < playerRadius) {
            ...
            if (frameCount % 30 === 0) {
                console.log('Collision detected!', intersections[0].distance.toFixed(2) + 'm');
            }
            break;
        }
    }
...
function animate() {
    ...
    logInterval += delta;
    if (logInterval >= 2.0) {
        console.log('Current Camera:', `X:${camera.position.x.toFixed(3)} Y:${camera.position.y.toFixed(3)} Z:${camera.position.z.toFixed(3)}`, `Locked: ${controls.isLocked}`);
        logInterval = 0;
    }
